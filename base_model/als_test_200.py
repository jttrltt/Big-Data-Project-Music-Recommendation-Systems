...
def main(spark, netID):
    ...
    max_iter = 20
    infile = 'cf_train_new'
    als = ALS.load(f'model/als_{rank}_{regParam}_{max_iter}_{scale}_{infile}')
    model = ALSModel.load(f'model/model_{rank}_{regParam}_{max_iter}_{scale}_{infile}')
    ...
    print(f'Rank:{rank}, regParam: {regParam}, alpha:{scale}, max_iter:{max_iter} ,map score:  {MAP}, ndcg score: {ndcg}, pk score: {pk}')
...
#     conf.set("spark.yarn.memoryOverhead","16G")
    ...
    spark = SparkSession.builder.appName('testing').config('spark.blacklist.enabled', False).getOrCreate()
